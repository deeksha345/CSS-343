...
/*
In graph.cpp hope ~Graph: fix if you get memory leaks 
Deeksha Sharma 2/13/2017 
Assumptions: 
...
// add the path to get to this vertex to global variable
// [A B]
...
void graphCostDisplayPath(string vertex) {
	...
	while (prevIt != previous.end()) {
		//cout << "prevIt->second: " << prevIt->second << endl;
...
void vertexTests()
{
	...
	cout << "a.connect(b.getLabel(), 10): " << 
		a.connect(b.getLabel(), 10) << endl;
	cout << "a.connect(c.getLabel(), 27): " << 
		a.connect(c.getLabel(), 27) << endl;
	cout << "a.connect(d.getLabel(), 5): " << 
		a.connect(d.getLabel(), 5) << endl;

	a.resetNeighbor();

	cout << "Edge weight for a to B: " << a.getEdgeWeight(b.getLabel()) << endl;
	cout << "Number of A's neighbors: " << a.getNumberOfNeighbors() << endl;
	cout << "A's next neighbor: " << a.getNextNeighbor() << endl;
	cout << "A's next neighbor: " << a.getNextNeighbor() << endl;
	a.resetNeighbor();
	cout << "A's next neighbor: " << a.getNextNeighbor() << endl;
	cout << "A == B: " << (a == b) << endl;
	...
	cout << "B < A: " << (b < a) << endl;
	cout << "B < C: " << (b < c) << endl;
	a.disconnect(b.getLabel());
	cout << "Is B a neighbot of A?: " << a.findNeighbor(b.getLabel()) << endl;
	cout << "Is C a neighbot of A?: " << a.findNeighbor(c.getLabel()) << endl;

	a.visit();
	cout << "is A visited: " << a.isVisited() << endl;
	a.unvisit();
	cout << "is A visited: " << a.isVisited() << endl;
}
